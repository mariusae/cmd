...
// (packages/core/src/indexing/block-context.test.ts), which is where these
// rules come from, carried over with `refl`'s reading of them: the sibling
// branches kept beside a match are the ones that matched the query too.
...
// Only `title:` is read out of frontmatter, and reading it never fails: broken
// or non-mapping YAML degrades to no title, never to an unreadable draft.
func TestFrontmatterTitle(t *testing.T) {
	for _, test := range []struct{ name, raw, want string }{
		{name: "a title is read", raw: "title: A Draft", want: "A Draft"},
		{name: "and trimmed", raw: "title:   A Draft  ", want: "A Draft"},
		{name: "another key is not a title", raw: "status: parked"},
		{name: "a non-string title is no title", raw: "title: 7"},
		...
		t.Run(test.name, func(t *testing.T) {
			if got := frontmatterTitle(test.raw); got != test.want {
