...
// Markdown block structure: the unit of meaning a match sits in, rather than
// the physical line it landed on. The rules are Reflect's block-context rules,
// ported here as `refl` ported them, because they are the rules that make a
// quotation read as something someone wrote:
//
//   - Paragraph: the whole paragraph, however it wraps.
//   - Heading: the heading plus every following sibling block up to the next
//     heading of any level.
//   - Title heading: just the heading line. A draft's title is its first H1,
//     so the section rule would otherwise quote the entire draft back.
...
func prepareBlocks(source string) *blockSource {
	...
	s := &blockSource{
		...
		spans:             map[ast.Node]span{},
		frontmatterTitled: frontmatterTitle(split.raw) != "",
	}
...
// preserving offsets. An unterminated fence is tolerated as body, so a draft
...
// frontmatterTitle reads a `title:` out of frontmatter, which is the one key
// drafts acts on. Parsing is tolerant: broken or non-mapping YAML degrades to
// no frontmatter, never to an unreadable draft.
